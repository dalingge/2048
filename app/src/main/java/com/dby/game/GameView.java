...
public class GameView  extends GridLayout implements View.OnTouchListener{
    ...
    public GameView(Context context) {
        super(context);
        mTarget = AppConfig.mSp.getInt(AppConfig.KEY_GAME_GOAL, 2048);
        initGameMatrix();
    }
    ...
    public void startGame() {
        initGameMatrix();
        initGameView(AppConfig.mItemSize);
    }
    ...
    private void initGameMatrix() {
        ...
        mScoreHistory = 0;
        AppConfig.SCROE = 0;
        AppConfig.mGameLines = AppConfig.mSp.getInt(AppConfig.KEY_GAME_LINES, 4);
        mGameLines = AppConfig.mGameLines;
        ...
        getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                AppConfig.mItemSize = getWidth()/ AppConfig.mGameLines;
                initGameView(AppConfig.mItemSize);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (isMoved()) {
                    addRandomNum();
                    // 修改显示分数
                    MainActivity.getmMainActivity().setScore(AppConfig.SCROE, 0);
                }
                checkCompleted();
                break;
            default:
...
//                            }).create().show();
    ...
    private void checkCompleted() {
        ...
        if (result == 0) {
            if (AppConfig.SCROE > mHighScore) {
                SharedPreferences.Editor editor = AppConfig.mSp.edit();
                editor.putInt(AppConfig.KEY_HIGH_SCROE, AppConfig.SCROE);
                editor.apply();
                MainActivity.getmMainActivity().setScore(AppConfig.SCROE, 1);
                AppConfig.SCROE = 0;
            }
                    ...
                    .setPositiveButton("重新开始",
                            new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface arg0,
                                    ...
                                    // 重新开始
                                    startGame();
                                    MainActivity.getmMainActivity().setScore(0, 0);
                                }
                            })
                    .setNegativeButton("继续",
                            new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface arg0,
                                    ...
                                    SharedPreferences.Editor editor = AppConfig.mSp.edit();
                                    if (mTarget == 1024) {
                                        editor.putInt(AppConfig.KEY_GAME_GOAL, 2048);
                                        mTarget = 2048;
                                        MainActivity.getmMainActivity().setGoal(2048);
                                    } else if (mTarget == 2048) {
                                        editor.putInt(AppConfig.KEY_GAME_GOAL, 4096);
                                        mTarget = 4096;
                                        MainActivity.getmMainActivity().setGoal(4096);
                                    } else {
                                        editor.putInt(AppConfig.KEY_GAME_GOAL, 4096);
                                        mTarget = 4096;
                                        MainActivity.getmMainActivity().setGoal(4096);
